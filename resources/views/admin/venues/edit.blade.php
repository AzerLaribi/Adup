    ...
    <div class="card-body">
        <form action="{{ route("admin.venues.update", [$venue->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('video') ? 'has-error' : '' }}">
                <label for="name">{{ trans('cruds.venue.fields.photos') }}*:</label>
            ...
            <div class="form-group {{ $errors->has('roles') ? 'has-error' : '' }}">
                <label for="rasbarys">rasbarys*
                    <span class="btn btn-info btn-xs select-all">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all">{{ trans('global.deselect_all') }}</span></label>
                <select name="rasbarys[]" id="rasbarys" class="form-control select2" multiple="multiple" >
                    @foreach($rasbarys as $id => $rasbarys)
                        <option value="{{ $id }}" {{ (in_array($id, old('rasbarys', [])) || isset($venue) && $venue->rasbarys->contains($id)) ? 'selected' : '' }}>{{ $rasbarys }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('roles') ? 'has-error' : '' }}">
                ...
                <select name="typs[]" id="typs" class="form-control select2" multiple="multiple" >
                    @foreach($typs as $id => $typs)
                        <option value="{{ $id }}" {{ (in_array($id, old('typs', [])) || isset($typs) && $venue->typs->contains($id)) ? 'selected' : '' }}>{{ $typs }}</option>
                    @endforeach
                </select>
