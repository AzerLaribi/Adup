            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-Locationpartner">
                ...
                <tr data-entry-id="{{ $advertiser->id }}">
                    ...
                    <td class="p-3 mb-2 bg-success text-white">
                        @if($advertiser->status==0) Waiting  @else Validate @endif
                    </td>
                    @else
                    ...
                    @endif
                    <td>
                                <a class="btn btn-xs btn-primary" href="{{ route('admin.advertiser.show', $advertiser->id) }}">
                                    {{ trans('global.view') }}
                                </a>
                                <form action="{{ route('admin.advertiser.destroy', $advertiser->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
